...
?>
<?php 
	include(drupal_get_path('module', 'weixin') .'/includes/weixin.menus.inc');
	...
	if(isset($menu_itmes_obj['menu']['button'])) {
		foreach ((array)$menu_itmes_obj['menu']['button'] as $key => $item) {
			$menu_data[$i] = array(
			  'name' => $item['name'],
			  'key_url' => isset($item['key'])?$item['key']:$item['url'],
			  'menu_type' => isset($item['key'])?0:1,
			  );
			$i++;
		}
	}
